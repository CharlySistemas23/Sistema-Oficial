...
const Settings = {
    ...
    getSystemTab() {
        return `
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--spacing-md);">
                <div class="module" style="padding: var(--spacing-md); background: var(--color-bg-card); border-radius: var(--radius-md); border: 1px solid var(--color-border-light);">
                    <h3 style="margin-bottom: var(--spacing-md); font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">
                        <i class="fas fa-server"></i> Servidor Centralizado
                    </h3>
                    ...
                    <button class="btn-primary btn-sm" onclick="if(typeof window.SettingsAPI !== 'undefined') window.SettingsAPI.showServerConfig(); else alert('Módulo de configuración del servidor no disponible');" style="width: 100%; margin-bottom: var(--spacing-xs);">
                        <i class="fas fa-cog"></i> Configurar Servidor
                    </button>
                    <button class="btn-secondary btn-sm" onclick="window.Settings.testServerConnection()" style="width: 100%; margin-bottom: var(--spacing-xs);">
                        <i class="fas fa-plug"></i> Probar Conexión
                    </button>
                    <div style="margin-top: var(--spacing-md); padding-top: var(--spacing-md); border-top: 1px solid var(--color-border-light);">
                        <h4 style="font-size: 12px; font-weight: 600; margin-bottom: var(--spacing-xs);">Sincronización</h4>
                        <div id="sync-queue-status" style="font-size: 11px; color: var(--color-text-secondary); margin-bottom: var(--spacing-xs);">
                            Pendientes: <span id="sync-queue-count">0</span>
                        </div>
                        <button class="btn-secondary btn-sm" onclick="if(typeof window.SyncManager !== 'undefined') window.SyncManager.syncPending(); else alert('Sync Manager no disponible');" style="width: 100%; margin-bottom: var(--spacing-xs);">
                ...
                if (apiUrl) {
                    ...
                    statusDiv.innerHTML = `
                        <div style="color: var(--color-warning);">
                            ...
                            <small style="color: var(--color-text-secondary);">Haz clic en "Configurar Servidor" para comenzar</small>
                        </div>
                    `;
                }
            }
    ...
    async testServerConnection() {
        try {
            const apiUrl = (await DB.get('settings', 'api_url'))?.value || null;
            if (!apiUrl) {
                Utils.showNotification('Servidor no configurado', 'warning');
                return;
            }
            ...
            if (response.ok) {
                ...
                Utils.showNotification(`✅ Conexión exitosa: ${data.status}`, 'success');
            } else {
