  ...
  if (FORCE_LOCAL_LOGIN) {
    ...
    await page.evaluate(async () => {
      ...
      if (typeof window.bypassLogin === 'function') {
        await Promise.race([
          Promise.resolve().then(() => window.bypassLogin()),
...
async function openModule(page, moduleName) {
  ...
  if (STATIC_MODULES.has(moduleName)) {
    ...
    await expect(page.locator('#module-content')).toBeVisible({ timeout: 20000 });
...
test.describe('E2E - Todos los módulos', () => {
  ...
  test('ejecuta acciones funcionales base por módulo crítico', async ({ page }) => {
    ...
    await expect(page.locator('#module-dashboard')).toContainText(/ventas|utilidad|resumen/i, { timeout: 10000 });
    ...
    await expect(page.locator('#module-pos')).toContainText(/venta|carrito|pago/i, { timeout: 10000 });
