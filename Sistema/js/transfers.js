...
const Transfers = {
    ...
    async loadTransfers() {
        ...
        try {
            ...
            // PASO 1: Subir transferencias locales que NO están en el servidor
            try {
                if (typeof API !== 'undefined' && API.baseURL && API.token && API.createTransfer) {
                    console.log('📤 [Paso 1 Transfers] Buscando transferencias locales que no están en el servidor...');
                    ...
                    console.log(`📊 [Paso 1 Transfers] Encontradas ${transfersToSync.length} transferencias locales sin sincronizar`);
                    ...
                    if (transfersToSync.length > 0) {
                        ...
                        for (const [key, localTransfer] of transfersByKey) {
                            try {
                                console.log(`📤 [Paso 1 Transfers] Subiendo transferencia local al servidor: ${localTransfer.id}`);
                                ...
                                console.error(`❌ [Paso 1 Transfers] Error subiendo transferencia ${localTransfer.id}:`, uploadError);
                            }
                        }
                        
                        console.log(`✅ [Paso 1 Transfers] Sincronización local→servidor completada: ${uploadedCount} transferencias subidas`);
                    }
                }
            } catch (error) {
                console.error('❌ [Paso 1 Transfers] Error sincronizando transferencias locales al servidor:', error);
            }

            // CACHE-FIRST: leer de IndexedDB primero para mostrar al instante
            const norm = (id) => String(id || '').trim().toLowerCase();
            const fromIdb = await DB.getAll('inventory_transfers') || [];
            let cachedFiltered = [];
            if (viewAllBranches) {
                cachedFiltered = fromIdb;
            } else if (filterBranchId) {
                const fb = norm(filterBranchId);
                cachedFiltered = fromIdb.filter(t => {
                    const fromB = t.from_branch_id ? norm(t.from_branch_id) : '';
                    const toB = t.to_branch_id ? norm(t.to_branch_id) : '';
                    return fromB === fb || toB === fb;
                });
                ...
                cachedFiltered = fromIdb.filter(t => {
                    const fromB = t.from_branch_id ? norm(t.from_branch_id) : '';
                    const toB = t.to_branch_id ? norm(t.to_branch_id) : '';
                    return fromB === cb || toB === cb;
                });
            } else {
                cachedFiltered = fromIdb;
            }
            const hasCache = cachedFiltered.length > 0;
            const hasApi = typeof API !== 'undefined' && API.baseURL && API.token && API.getTransfers;

            if (hasCache) {
                allTransfers = cachedFiltered;
                console.log(`⚡ [Cache-First] ${allTransfers.length} transferencias desde IndexedDB`);
                if (hasApi) {
                    Promise.resolve().then(async () => {
                        try {
                            const fresh = await API.getTransfers({ branch_id: filterBranchId });
                            for (const t of fresh) {
                                try {
                                    await DB.put('inventory_transfers', { ...t, server_id: t.id, sync_status: 'synced' }, { autoBranchId: false });
                                } catch (e) {}
                            }
                            const beforeIds = new Set((allTransfers || []).map(t => t.id));
                            const afterIds = new Set(fresh.map(t => t.id));
                            const changed = beforeIds.size !== afterIds.size || [...beforeIds].some(id => !afterIds.has(id));
                            const now = Date.now();
                            if (changed && (now - this._lastBackgroundRefreshTime) >= 3000) {
                                this._lastBackgroundRefreshTime = now;
                                await this.loadTransfers();
                            }
                        } catch (e) { console.warn('Sync transfers background:', e); }
                    }).catch(e => console.warn('Sync transfers background:', e));
                }
            } else if (hasApi) {
                try {
                    console.log('📥 [Paso 2 Transfers] Sincronizando transferencias desde el servidor...');
                    allTransfers = await API.getTransfers({ branch_id: filterBranchId });
                    console.log(`📥 [Paso 2 Transfers] ${allTransfers.length} transferencias recibidas del servidor`);
                    ...
                    for (const serverTransfer of allTransfers) {
                        try {
                            ...
                            console.warn(`⚠️ [Paso 2 Transfers] Error guardando transferencia ${serverTransfer.id}:`, error);
                        }
                    }
                    
                    console.log(`✅ [Paso 2 Transfers] Sincronización servidor→local completada: ${savedCount} nuevas, ${updatedCount} actualizadas`);
                    ...
                    if (filterBranchId) {
                        ...
                        console.log(`📍 Transfers: Filtrado estricto API: ${beforeStrictFilter} → ${allTransfers.length} (sucursal: ${filterBranchId})`);
                    }
                } catch (apiError) {
            ...
            console.log(`🔍 [Paso 3 Transfers] Deduplicación: ${allTransfers.length} transferencias únicas`);
            
            // Aplicar filtro de sucursal si es necesario (solo si no usamos API)
            if (typeof API === 'undefined' || !API.baseURL || !API.token) {
                try {
                    // Filtrar por sucursal si hay filtro específico - ESTRICTO
                    if (filterBranchId) {
                        ...
                        allTransfers = allTransfers.filter(t => {
                            ...
                            // CRÍTICO: Excluir transferencias sin branch_id cuando se filtra por sucursal específica
                            if (!fromBranch && !toBranch) {
                                return false; // NO mostrar transferencias sin sucursales
                            }
                            return fromBranch === normalizedBranchId || toBranch === normalizedBranchId;
                        });
                        console.log(`📍 Filtrado de transferencias por sucursal: ${beforeCount} → ${allTransfers.length} (sucursal: ${filterBranchId})`);
                    }
                } catch (dbError) {
                    console.error('Error obteniendo transferencias de DB:', dbError);
                    allTransfers = [];
    ...
    async createTransfer() {
        ...
        try {
            ...
            if (typeof API !== 'undefined' && API.baseURL && API.token && API.createTransfer) {
                try {
                    console.log('📦 Creando transferencia en servidor...');
                    transfer = await API.createTransfer(transferData);
                    console.log('✅ Transferencia creada en servidor:', transfer.folio);
                    createdViaAPI = true;
                } catch (apiError) {
                    console.warn('⚠️ Error creando transferencia en servidor (continuando con modo local):', apiError);
            ...
            if (!transfer) {
                ...
                console.log('✅ Transferencia guardada en IndexedDB');
            } else {
                // Si se creó con API, también guardar en IndexedDB para sincronización bidireccional
                await DB.put('inventory_transfers', transfer, { autoBranchId: false });
                console.log('✅ Transferencia guardada en IndexedDB (desde servidor)');
            }
    ...
    setupSocketListeners() {
        ...
        if (typeof UserManager !== 'undefined' && UserManager.currentUser?.is_master_admin) {
            window.addEventListener('transfer-updated-all-branches', async (e) => {
                ...
                if (this.initialized) {
                    console.log(`📦 Transfers: Transferencia actualizada en sucursal ${branchId} (${action})`);
                    // Recargar transferencias después de un pequeño delay
                    setTimeout(async () => {
                        await this.loadTransfers();
                    }, 300);
                }
        ...
        window.addEventListener('transfer-updated', async (e) => {
            if (this.initialized) {
                ...
                if (transfer && currentBranchId && 
                    ...
                    console.log(`📦 Transfers: Transferencia actualizada localmente`);
                    setTimeout(async () => {
                        await this.loadTransfers();
                    }, 300);
                } else if (!currentBranchId) {
