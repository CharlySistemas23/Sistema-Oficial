...
const ProfitCalculator = {
    ...
    async calculateMonthlyProfit(monthDate, branchId = null) {
        try {
            ...
            // 5. LLEGADAS: Costos de llegadas — usar cost_entries como fuente autorizada (igual que calculateDailyProfit)
            let arrivalCosts = 0;
            if (paidArrivalEntries.length > 0) {
                arrivalCosts = paidArrivalEntries.reduce((sum, c) => sum + parseAmt(c.amount), 0);
            } else {
                // Fallback: calcular desde agency_arrivals si no hay costos registrados en cost_entries
                const arrivals = await DB.getAll('agency_arrivals') || [];
                const monthArrivals = arrivals.filter(a => {
                    if (!a.date) return false;
                    const arrivalDate = new Date(a.date);
                    if (arrivalDate.getMonth() !== month || arrivalDate.getFullYear() !== year) return false;
            ...
            // 6. COMISIONES BANCARIAS — usar cost_entries filtradas por branchId (igual que calculateDailyProfit)
            let bankCommissions = 0;
            if (typeof Costs !== 'undefined') {
                const bankCommEntries = (await Costs.getFilteredCosts({
                    branchId: branchId,
                    dateFrom: monthStartStr,
                    dateTo: monthEndStr,
                    category: 'comisiones_bancarias'
                })) || [];
                bankCommissions = bankCommEntries.reduce((sum, c) => sum + parseAmt(c.amount), 0);
            } else {
                // Fallback sin filtro de sucursal (solo un lugar de negocio)
                const payments = await DB.getAll('payments') || [];
                bankCommissions = payments
                    .filter(p => {
    ...
    async calculateDailyProfit(dateYYYYMMDD, branchId) {
        try {
            // Validar que branchId exista — master_admin sin sucursal específica: saltar
            if (!branchId) {
                console.warn('calculateDailyProfit: branchId no proporcionado, omitiendo cálculo');
                return null;
            }
