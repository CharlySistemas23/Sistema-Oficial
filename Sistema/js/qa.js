...
const QA = {
    ...
    isAdmin() {
        ...
        return user && (user.role === 'admin' || user.role === 'master_admin' || user.permissions?.includes('all'));
    },
        ...
        content.innerHTML = `
            <div class="qa-container" style="padding: 20px;">
                ...
                <div class="qa-controls" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 10px; margin-bottom: 16px;">
                    <button class="btn-primary" id="qa-btn-global-test" onclick="QA.runGlobalSystemTest()" style="background: linear-gradient(135deg, #10b981, #059669); border: none;">
                        <i class="fas fa-globe"></i> Prueba Global Railway
                    </button>
    ...
    async checkSalesPayments() {
        ...
        for (const sale of sales) {
            if (!(typeof Utils !== 'undefined' && Utils.isSaleCompleted ? Utils.isSaleCompleted(sale) : (sale.status === 'completada' || sale.status === 'completed'))) continue;
    ...
    async checkInventoryStatus() {
        ...
        for (const item of items) {
            ...
            const soldSales = soldInItems.map(si => sales.find(s => s.id === si.sale_id && (typeof Utils !== 'undefined' && Utils.isSaleCompleted ? Utils.isSaleCompleted(s) : (s.status === 'completada' || s.status === 'completed'))));
    ...
    async exportReport() {
        ...
        UI.showModal('Exportar Reporte QA', `
            <div style="display: grid; gap: 12px;">
                <button class="btn-secondary" onclick="UI.closeModal(); QA.exportReportPDF();">
                    <i class="fas fa-file-pdf"></i> Exportar PDF
                </button>
                <button class="btn-secondary" onclick="UI.closeModal(); QA.exportReportExcel();">
                    <i class="fas fa-file-excel"></i> Exportar Excel
                </button>
                <button class="btn-secondary" onclick="UI.closeModal(); QA.exportReportCSV();">
                    <i class="fas fa-file-csv"></i> Exportar CSV
                </button>
                <button class="btn-secondary" onclick="UI.closeModal(); QA.exportReportJSON();">
    ...
    exportReportExcel() {
        ...
        try {
            ...
            Utils.showNotification('Error al exportar Excel', 'error');
        } finally {
            this.isExporting = false;
        }
    },
    ...
    exportReportCSV() {
        ...
        try {
            ...
            Utils.showNotification('Error al exportar CSV', 'error');
        } finally {
            this.isExporting = false;
        }
    },
    ...
    async verifyMathematicalIntegrity() {
        ...
        try {
            ...
            // 5. Verificar utilidad diaria con campos canónicos
            const profitReports = await DB.getAll('daily_profit_reports');
            for (const report of profitReports) {
                const revenue = parseFloat(report.revenue_sales_total ?? report.revenue ?? 0) || 0;
                const cogs = parseFloat(report.cogs_total ?? 0) || 0;
                const commissionsSellers = parseFloat(report.commissions_sellers_total ?? 0) || 0;
                const commissionsGuides = parseFloat(report.commissions_guides_total ?? 0) || 0;
                const arrivals = parseFloat(report.arrivals_total ?? 0) || 0;
                const fixedCosts = parseFloat(report.fixed_costs_daily ?? 0) || 0;
                const variableCosts = parseFloat(report.variable_costs_daily ?? 0) || 0;
                const bankCommissions = parseFloat(report.bank_commissions ?? 0) || 0;

                const calculatedProfit = revenue - (
                    cogs + commissionsSellers + commissionsGuides + arrivals + fixedCosts + variableCosts + bankCommissions
                );
                const reportedProfit = parseFloat(report.profit_before_taxes ?? report.net_profit ?? 0) || 0;

                if (Math.abs(reportedProfit - calculatedProfit) > 0.01) {
                    results.issues.push({
                        type: 'profit_mismatch',
                        reportId: report.id,
                        date: report.date,
                        expectedProfit: calculatedProfit.toFixed(2),
                        reportedProfit: reportedProfit.toFixed(2)
                    });
                    results.failed++;
                } else {
                    results.passed++;
    ...
    // =====================================================
    // PRUEBA GLOBAL DEL SISTEMA (Railway)
    // =====================================================
    async runGlobalSystemTest() {
        this.addLog('info', '🌐 Iniciando Prueba Global del Sistema...');
        this.addLog('info', 'Verificando conexión con Railway...');
        ...
        if (!apiUrl) {
            this.addLog('error', '❌ API no configurado. Ve a Configuración → Sincronización');
            Utils.showNotification('API no configurado', 'error');
            return;
        }
        
        if (!hasToken) {
            this.addLog('warning', '⚠️ URL configurada pero no hay sesión activa. Inicia sesión con admin / 1234');
            Utils.showNotification('Inicia sesión para conectar con Railway', 'warning');
            return;
        }
        
        this.addLog('success', `✅ API configurado: ${apiUrl}`);
        
        // Ejecutar la prueba global usando la función del script test-global-system.js
        if (typeof window.testGlobalSystem === 'function') {
            try {
                const results = await window.testGlobalSystem();
                
                // Mostrar resultados en el log de QA
                this.addLog('info', '═'.repeat(40));
                this.addLog('info', '📊 RESULTADOS DE LA PRUEBA GLOBAL');
                this.addLog('info', '═'.repeat(40));
                
                for (const [module, result] of Object.entries(results.modules)) {
                    if (result.tested) {
                        if (result.success > 0) {
                            this.addLog('success', `✅ ${module}: ${result.success} éxito(s)`);
                        } else if (result.errors.length > 0) {
                            this.addLog('error', `❌ ${module}: ${result.errors.length} error(es)`);
                ...
                this.addLog('info', '═'.repeat(40));
                this.addLog('info', `Total módulos probados: ${Object.keys(results.modules).length}`);
                this.addLog('success', `✅ Éxitos: ${results.success}`);
                if (results.errors > 0) {
                    this.addLog('error', `❌ Errores: ${results.errors}`);
                }
                
                if (results.success > 0) {
                    this.addLog('success', '🎉 ¡Prueba completada! Los datos se han guardado en Railway');
                    Utils.showNotification(`Prueba completada: ${results.success} módulos exitosos`, 'success');
                } else {
                    this.addLog('warning', '⚠️ No se pudo probar ningún módulo');
                    Utils.showNotification('Prueba fallida: revisa los errores', 'error');
                }
                
            } catch (error) {
                this.addLog('error', `❌ Error ejecutando prueba global: ${error.message}`);
                console.error('Error en prueba global:', error);
                Utils.showNotification('Error ejecutando prueba global', 'error');
            }
        } else {
            this.addLog('error', '❌ Función testGlobalSystem() no disponible. Asegúrate de que test-global-system.js esté cargado.');
            Utils.showNotification('Script de prueba global no disponible', 'error');
        }
    },
