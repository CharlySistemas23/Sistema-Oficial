    ...
    async updateBranchSelector() {
        ...
        this._updateBranchSelectorPromise = (async () => {
            ...
            if (isMasterAdmin) {
                ...
                // Deduplicar por id: evita botones dobles si IDB tiene registros repetidos.
                // Las sucursales con mismo nombre pero distinto ID son un problema de BD — no las ocultar.
                const uniqueById = new Map();
                for (const b of branches) {
                    if (b?.id && !uniqueById.has(b.id)) uniqueById.set(b.id, b);
                }
                const activeBranches = [...uniqueById.values()].filter(b => b.active !== false);
