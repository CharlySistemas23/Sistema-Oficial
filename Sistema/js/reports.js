...
const Reports = {
    ...
    async displayReport(sales, saleItems = [], items = []) {
        ...
        if (completedSales.length > 0) {
            ...
            // DEDUPE: hay un bug en el POS/sync que crea multiples cost_entries por
            // la misma venta. Visto en DB: una sola comision aparece hasta 11 veces.
            // Hasta que se arregle el origen, el reporte deduplica por (category, notes,
            // amount, date) y cuenta cada concepto una sola vez. Esto da utilidad correcta
            // aunque la DB tenga duplicates.
            const reportCosts = (() => {
                const seen = new Map();
                for (const c of (rawReportCosts || [])) {
                    const cat = String(c.category || '').toLowerCase();
                    // Solo deduplicamos categorias auto-generadas por el POS
                    // (los costos manuales fijos/variables no deben deduparse asi).
                    const isAutoGenerated = cat === 'costo_ventas' || cat === 'comisiones' ||
                                            cat === 'pago_llegadas' || cat === 'comisiones_bancarias';
                    if (!isAutoGenerated) {
                        seen.set(c.id || `manual-${seen.size}`, c);
                        continue;
                    }
                    const dateStr = (c.date || c.created_at || '').toString().split('T')[0];
                    const key = `${cat}|${(c.notes || c.description || '').trim()}|${parseFloat(c.amount) || 0}|${dateStr}`;
