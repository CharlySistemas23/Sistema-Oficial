    ...
    async archiveQuickCaptureReport() {
        try {
            ...
            Object.values(salesByGuide).forEach(guideData => {
                // Obtener pasajeros de la agencia del guía
                const guideAgencyPassengers = filteredArrivals
                    .filter(a => a.agency_id === guideData.agency_id)
                    .reduce((sum, a) => sum + (a.passengers || 0), 0);
                
                // Si hay múltiples guías de la misma agencia, dividir pasajeros proporcionalmente
                const guidesOfSameAgency = Object.values(salesByGuide).filter(g => g.agency_id === guideData.agency_id);
                const totalVentasAgencia = guidesOfSameAgency.reduce((sum, g) => sum + g.ventas, 0);
                const pasajerosAsignados = totalVentasAgencia > 0 
                    ? Math.round((guideData.ventas / totalVentasAgencia) * guideAgencyPassengers)
                    : 0;

                const ticketPromedio = guideData.ventas > 0 ? guideData.total_ventas_mxn / guideData.ventas : 0;
                const cierrePercent = pasajerosAsignados > 0 ? ((guideData.ventas / pasajerosAsignados) * 100) : 0;
