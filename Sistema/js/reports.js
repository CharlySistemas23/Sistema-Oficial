...
const Reports = {
    ...
    async displayReport(sales, saleItems = [], items = []) {
        ...
        if (completedSales.length > 0) {
            ...
            if (typeof Costs !== 'undefined') {
            const rawReportCosts = await Costs.getFilteredCosts({
                branchId: branchId || null,
                dateFrom: costsDateFrom,
                dateTo: costsDateTo
            });

            // DEDUPE: hay un bug grave en el POS/sync/scripts pasados que crea multiples
            // cost_entries identicos. Visto en DB:
            //   - Una sola comision aparece hasta 11 veces (bug del POS)
            //   - Una nomina semanal aparece 6,050 veces el mismo dia (script roto que
            //     corrio en 1 segundo el 2026-03-07)
            // Hasta que se limpie la DB, el reporte deduplica TODAS las categorias por
            // (branch_id, category, notes, amount, date). Es seguro porque dos cost_entries
            // con EXACTAMENTE la misma firma son inequivocamente duplicados.
            const reportCosts = (() => {
                const seen = new Map();
                for (const c of (rawReportCosts || [])) {
                    const cat = String(c.category || '').toLowerCase().trim();
                    const dateStr = (c.date || c.created_at || '').toString().split('T')[0];
                    const notesNorm = (c.notes || c.description || '').toString().trim();
                    const branchKey = String(c.branch_id || '').trim();
                    const amountNum = parseFloat(c.amount) || 0;
                    const key = `${branchKey}|${cat}|${notesNorm}|${amountNum}|${dateStr}`;
    ...
    async displayDailyReport(sales) {
        ...
        const calculateDailyCosts = async (dateStr, branchId) => {
            ...
            // Costos mensuales prorrateados — dedupe por gasto logico para evitar
            // que el mismo costo registrado N veces se cuente N veces.
            ...
            for (const cost of monthlyCosts) {
                ...
                fixedCostsDaily += (parseFloat(cost.amount) || 0) / DAYS_PER_MONTH;
            }
            ...
            for (const cost of weeklyCosts) {
                fixedCostsDaily += (parseFloat(cost.amount) || 0) / 7;
            }
            ...
            for (const cost of annualCosts) {
                ...
                fixedCostsDaily += (parseFloat(cost.amount) || 0) / daysInYear;
            }
    ...
    async deleteHistoricalReport(reportId) {
        try {
            ...
            // Cargar el reporte local para obtener server_id y la clave de busqueda
            let localReport = null;
            try { localReport = await DB.get('historical_reports', reportId); } catch (_) {}

            const serverIdsToTry = new Set();
            if (localReport?.server_id) serverIdsToTry.add(localReport.server_id);
            if (typeof reportId === 'string' && reportId && !reportId.startsWith('historical_')) {
                serverIdsToTry.add(reportId);
            }

            const tryDelete = async (id) => {
                try {
                    await API.deleteHistoricalReport(id);
                    console.log(`✅ Reporte histórico ${id} eliminado del servidor`);
                    return { ok: true, status: 200 };
                } catch (error) {
                    const status = error?.status;
                    if (status === 404 || (error?.message || '').includes('no encontrado')) {
                        return { ok: false, status: 404 };
                    }
                    console.warn(`Error eliminando ${id}:`, error);
            ...
            if (typeof API !== 'undefined' && API.deleteHistoricalReport) {
                // Intentar con los ids conocidos
                for (const id of serverIdsToTry) {
                    const res = await tryDelete(id);
                    if (res.ok) { serverDeleted = true; break; }
                }

                // Si todos los intentos dieron 404 o no hay id, buscar en el servidor
                // por la clave (period_type + date_from + date_to + branch_id) y borrar
                // con el id real. Esto evita que el Paso 2 de loadHistoricalReports
                // re-cree el reporte local porque sigue existiendo en el servidor.
                if (!serverDeleted && localReport && API.getHistoricalReports) {
                    try {
                        const filters = {
                            period_type: localReport.period_type,
                            date_from: localReport.date_from,
                        ...
                        if (localReport.branch_id) filters.branch_id = localReport.branch_id;
                        const matches = await API.getHistoricalReports(filters) || [];
                        for (const m of matches) {
                            if (!m?.id || serverIdsToTry.has(m.id)) continue;
                            const res = await tryDelete(m.id);
                            if (res.ok) { serverDeleted = true; }
                        }
                    } catch (lookupErr) {
                        console.warn('No se pudo buscar el reporte en el servidor para borrarlo:', lookupErr);
                    }
                }
            }
