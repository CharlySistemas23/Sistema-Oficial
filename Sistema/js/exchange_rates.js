...
var ExchangeRates = window.ExchangeRates || {
    ...
    async getExchangeRate(dateStr = null) {
        try {
            ...
            // Si no existe, buscar en settings o en el último tipo de cambio guardado en localStorage
            const usdSetting = await DB.get('settings', 'exchange_rate_usd');
            const cadSetting = await DB.get('settings', 'exchange_rate_cad');
            const lastUsd = parseFloat(usdSetting?.value || localStorage.getItem('last_exchange_rate_usd') || localStorage.getItem('daily_exchange_rate') || 20.00);
            const lastCad = parseFloat(cadSetting?.value || localStorage.getItem('last_exchange_rate_cad') || 15.00);
            ...
            const lastUsd = parseFloat(localStorage.getItem('last_exchange_rate_usd') || localStorage.getItem('daily_exchange_rate') || 20.00);
            const lastCad = parseFloat(localStorage.getItem('last_exchange_rate_cad') || 15.00);
            return {
                usd: lastUsd,
                cad: lastCad,
    ...
    async saveExchangeRate(dateStr, usd, cad) {
        try {
            ...
            if (apiSaved) {
                ...
                localStorage.setItem('last_exchange_rate_usd', usd.toString());
                localStorage.setItem('last_exchange_rate_cad', cad.toString());
            ...
            // También actualizar settings y localStorage para compatibilidad y fallback offline
            await DB.put('settings', { key: 'exchange_rate_usd', value: usd, updated_at: new Date().toISOString() }, { autoBranchId: false });
            await DB.put('settings', { key: 'exchange_rate_cad', value: cad, updated_at: new Date().toISOString() }, { autoBranchId: false });
            localStorage.setItem('daily_exchange_rate', usd.toString());
            localStorage.setItem('last_exchange_rate_usd', usd.toString());
