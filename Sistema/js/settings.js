...
const Settings = {
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--spacing-md);">
                ...
                <div class="module" style="padding: var(--spacing-md); background: var(--color-bg-card); border-radius: var(--radius-md); border: 1px solid var(--color-border-light);">
                    ...
                    <div style="display: flex; flex-direction: column; gap: var(--spacing-xs);">
                        ...
                        <button class="btn-primary btn-sm" onclick="window.Settings.reconcileHistoricalProfits()" style="width: 100%; margin-top: var(--spacing-xs); background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; font-weight: 600; border: none;">
                            <i class="fas fa-chart-line"></i> Reconciliar Utilidades Históricas
                        </button>
                    </div>
                </div>
    ...
    async reconcileHistoricalProfits() {
        try {
            if (typeof ProfitCalculator === 'undefined' || !ProfitCalculator.rebuildHistoricalDailyReports) {
                Utils.showNotification('El motor de reconciliación no está disponible.', 'error');
                return;
            }

            const confirmed = await Utils.confirm(
                'Esto recalculará utilidades diarias históricas para todas las sucursales y fechas detectadas.\n\n¿Deseas continuar?'
            );
            if (!confirmed) return;

            const started = Date.now();
            let lastProgressNotified = 0;
            Utils.showNotification('Iniciando reconciliación histórica...', 'info');

            const result = await ProfitCalculator.rebuildHistoricalDailyReports({
                onProgress: (progress) => {
                    if (!progress || !progress.total) return;
                    const shouldNotify =
                        progress.processed === progress.total ||
                        progress.percent - lastProgressNotified >= 10;
                    if (!shouldNotify) return;
                    lastProgressNotified = progress.percent;
                    Utils.showNotification(
                        `Reconciliación en progreso: ${progress.percent}% (${progress.processed}/${progress.total})`,
                        'info'
                    );
                }
            });
            ...
            const successRate = result.scope?.operations
                ? Math.round((result.success / result.scope.operations) * 100)
                : 0;

            const errorItems = (result.errors || []).slice(0, 25).map(err => `
                <li style="margin-bottom: var(--spacing-xs);">
                    <strong>${err.date}</strong> · ${err.branchId}<br>
                    <span style="color: var(--color-text-secondary);">${err.message}</span>
                </li>
            `).join('');

            const reportHTML = `
                <div style="max-width: 760px;">
                    <h4 style="margin-bottom: var(--spacing-md);">Resultado de Reconciliación Histórica</h4>
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--spacing-sm); margin-bottom: var(--spacing-md);">
                        <div style="padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm); font-size: 12px;">
                            <strong>Operaciones</strong><br>${result.scope?.operations || 0}
                        </div>
                        <div style="padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm); font-size: 12px;">
                            <strong>Éxito</strong><br>${result.success} (${successRate}%)
                        </div>
                        <div style="padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm); font-size: 12px;">
                            <strong>Fallidas</strong><br>${result.failed}
                        </div>
                        <div style="padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm); font-size: 12px;">
                            <strong>Duración</strong><br>${durationSec}s
                        </div>
                    </div>

                    <div style="margin-bottom: var(--spacing-md); padding: var(--spacing-md); background: ${result.failed > 0 ? '#fff3e0' : '#e8f5e9'}; border-left: 4px solid ${result.failed > 0 ? '#f57c00' : '#388e3c'}; border-radius: var(--radius-sm); font-size: 12px;">
                        ${result.failed > 0
                            ? `<strong>Se completó con incidencias.</strong> Revisa el listado de errores para depurar registros puntuales.`
                            : `<strong>Reconciliación completada correctamente.</strong> Dashboard, reportes y costos quedan recalculados bajo el contrato canónico.`}
                    </div>

                    ${result.failed > 0 ? `
                        <div style="max-height: 280px; overflow-y: auto; padding: var(--spacing-sm); background: var(--color-bg-secondary); border-radius: var(--radius-sm);">
                            <h5 style="margin-bottom: var(--spacing-sm);">Errores (máximo 25)</h5>
                            <ul style="margin: 0; padding-left: var(--spacing-md); font-size: 12px;">
                                ${errorItems}
                            </ul>
                        </div>
            ...
            UI.showModal('Reconciliación Histórica de Utilidades', reportHTML, [
                { text: 'Cerrar', class: 'btn-primary', onclick: () => UI.closeModal() }
            ]);

            if (result.failed > 0) {
                Utils.showNotification(`Reconciliación terminada con ${result.failed} incidencias`, 'warning');
            } else {
                Utils.showNotification('Reconciliación histórica completada correctamente', 'success');
            }
        } catch (e) {
            console.error('Error en reconciliación histórica:', e);
            Utils.showNotification('Error al reconciliar utilidades: ' + e.message, 'error');
        }
    },
