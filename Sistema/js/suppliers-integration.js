...
window.SuppliersIntegration = {
    ...
    async createPOFromComparison(supplierId, itemId, price) {
        ...
        const quantity = await new Promise((resolve) => {
            const defaultVal = String(item.stock_minimo || 1);
            UI.showModal(
                'Cantidad de Orden',
                `<div style="padding: var(--spacing-md);">
                    <label>Ingresa la cantidad a ordenar:</label>
                    <input type="number" id="po-quantity-input" class="form-input"
                           value="${defaultVal}" min="1" style="margin-top: var(--spacing-sm);">
                </div>`,
                [
                    { text: 'Cancelar', class: 'btn-secondary', onclick: () => { UI.closeModal(); resolve(null); } },
                    { text: 'Confirmar', class: 'btn-primary', onclick: () => {
                        const v = document.getElementById('po-quantity-input')?.value;
                        UI.closeModal();
                        resolve(v);
                    }}
                ]
            );
            setTimeout(() => document.getElementById('po-quantity-input')?.focus(), 100);
        });
