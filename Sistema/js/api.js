...
const API = {
    ...
    async createInventoryItem(item) {
        ...
        // Campos del esquema: sku, barcode, name, description, category, metal, stone_type, stone_weight, weight, price, cost, stock_actual, stock_min, stock_max, status, branch_id, certificate_number, photos
        const backendFields = ['sku', 'barcode', 'name', 'description', 'category', 'metal', 'stone_type', 'stone_weight', 'weight', 'price', 'cost', 'stock_actual', 'stock_min', 'stock_max', 'status', 'branch_id', 'certificate_number', 'photos'];
        const cleanPayload = {};
    ...
    async updateInventoryItem(id, item) {
        ...
        // Solo enviar campos que existen en la base de datos
        // Campos del esquema: name, description, category, metal, stone_type, stone_weight, weight, price, cost, stock_actual, stock_min, stock_max, status, certificate_number, photos, barcode
        const backendFields = ['name', 'description', 'category', 'metal', 'stone_type', 'stone_weight', 'weight', 'price', 'cost', 'stock_actual', 'stock_min', 'stock_max', 'status', 'certificate_number', 'photos', 'barcode'];
        const cleanPayload = {};
        backendFields.forEach(field => {
            if (payload[field] !== undefined) {
                cleanPayload[field] = payload[field];
    ...
    async createGuide(guide) {
        ...
        const payload = { ...(guide || {}) };
        if (payload.id && !isUUID(payload.id)) delete payload.id;
        if (payload.agency_id && !isUUID(payload.agency_id)) delete payload.agency_id;
