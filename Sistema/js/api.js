...
const API = {
    ...
    async init() {
        const DEFAULT_API_URL = 'https://backend-production-6260.up.railway.app';
        ...
        try {
            ...
            // Si no hay URL guardada, usar la URL Railway por defecto y persistirla
            if (!savedURL) {
                this.baseURL = DEFAULT_API_URL;
                await DB.put('settings', { key: 'api_url', value: DEFAULT_API_URL });
                console.log(`🔗 URL por defecto configurada: ${DEFAULT_API_URL}`);
            }
            ...
            if (!this._initLogShown) {
                if (!this.baseURL) {
                    ...
                    if (!this.token) {
                        console.log('ℹ️ Sin token de sesión. El usuario debe iniciar sesión manualmente para conectar con el servidor.');
                        console.log('ℹ️ El sistema funcionará en modo local hasta que el usuario inicie sesión.');
                    } else {
    ...
    async getInventoryItems(filters = {}) {
        // Construir query string con todos los filtros posibles
        const params = new URLSearchParams();
        const add = (key, val) => { if (val !== undefined && val !== null && val !== '') params.set(key, val); };

        add('branch_id',      filters.branch_id);
        add('status',         filters.status);
        add('search',         filters.search);
        add('category',       filters.category);
        add('metal',          filters.metal);
        add('stone_type',     filters.stone_type);
        add('min_price',      filters.min_price);
        add('max_price',      filters.max_price);
        add('material',       filters.material);
        add('purity',         filters.purity);
        add('plating',        filters.plating);
        add('style',          filters.style);
        add('finish',         filters.finish);
        add('theme',          filters.theme);
        add('condition',      filters.condition);
        add('location_detail',filters.location_detail);
        add('collection',     filters.collection);
        add('updated_after',  filters.updated_after);
        add('page',           filters.page);
        add('limit',          filters.limit);
        // Param heredado por compatibilidad
        if (filters.sku)  add('search', filters.sku);

        const qs = params.toString();
        return await this.get(`/api/inventory${qs ? '?' + qs : ''}`);
    },
