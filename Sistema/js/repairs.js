...
const Repairs = {
    ...
    setupUI() {
        ...
        content.innerHTML = `
            <div class="module dashboard-card-prodex module-section" style="padding: var(--spacing-md); margin-bottom: var(--spacing-md);">
    ...
    async loadRepairs() {
        try {
            ...
            // PASO 2: Descargar reparaciones del servidor
            let repairs = [];
    ...
    async showAddForm(repairId = null) {
        ...
        UI.showModal(repairId ? 'Editar Reparación' : 'Nueva Reparación', body, footer, { size: 'large' });
    ...
    async saveRepair(repairId) {
        // Verificar permiso según si es creación o edición
        const requiredPermission = repairId ? 'repairs.edit' : 'repairs.create';
        if (typeof PermissionManager !== 'undefined' && !PermissionManager.hasPermission(requiredPermission)) {
            Utils.showNotification('No tienes permiso para realizar esta acción', 'error');
            return;
        }
    ...
    async showDetails(repairId) {
        ...
        UI.showModal(`Reparación: ${repair.folio}`, body, footer, { size: 'lg' });
    },
