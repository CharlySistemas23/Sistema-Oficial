        ...
        try {
            ...
            if (typeof API !== 'undefined' && API.baseURL && API.token && API.getRepairs) {
                try {
                    ...
                    // Esto asegura que reparaciones sin branch_id se excluyan cuando se filtra por sucursal específica
                    if (filterBranchId) {
                        const beforeStrictFilter = repairs.length;
                        repairs = repairs.filter(r => {
                            // CRÍTICO: Excluir reparaciones sin branch_id cuando se filtra por sucursal específica
                            if (!r.branch_id) {
                                return false; // NO mostrar reparaciones sin branch_id
                            }
                    ...
                    for (const repair of repairs) {
                        await DB.put('repairs', repair, { autoBranchId: false });
                    }
                    
                    console.log(`✅ ${repairs.length} reparaciones cargadas desde API (después de filtro estricto)`);
                } catch (apiError) {
