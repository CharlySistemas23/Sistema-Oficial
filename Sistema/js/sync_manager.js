...
const SyncManager = {
    ...
    async syncPending() {
        ...
        const now = Date.now();
        if (this._syncCooldownUntil && now < this._syncCooldownUntil) {
            return;
        }

        const scheduleSyncRetry = (baseMs, reason = 'retry') => {
            const jitterFactor = 0.85 + (Math.random() * 0.5); // 0.85x a 1.35x
            const waitMs = Math.max(1000, Math.floor(baseMs * jitterFactor));
            this._syncCooldownUntil = Date.now() + waitMs;
            console.warn(`⏳ Sync en cooldown por ${Math.ceil(waitMs / 1000)}s (${reason})`);
            setTimeout(() => {
                try {
                    this.syncPending();
                } catch (e) {}
        ...
        for (const item of toSync) {
            try {
                ...
                switch (item.type) {
                    ...
                    case 'inventory_item': // También manejar 'inventory_item' usado en inventory.js
                        ...
                        if (item.data === 'delete') {
                            ...
                            const isUUID = (value) => /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(value || ''));
                            const canUpdateById = isUUID(entityData.id);
                            let syncedEntity = null;

                            // Estrategia: intentar update directo (si hay UUID) y hacer fallback a create solo si no existe.
                            // Evita una llamada extra GET /inventory/:id por cada elemento en cola.
                            if (canUpdateById && typeof API.updateInventoryItem === 'function') {
                                try {
                                    syncedEntity = await API.updateInventoryItem(entityData.id, entityData);
                                } catch (updateError) {
                                    const status = updateError?.status || updateError?.cause?.status;
                                    const msg = String(updateError?.message || '').toLowerCase();
                                    const notFound = status === 404 || msg.includes('no encontrado');
                                    if (!notFound) {
                                        throw updateError;
                                    }
                                }
                            ...
                            if (!syncedEntity) {
                                ...
                                syncedEntity = await API.createInventoryItem(entityData);
                            }

                            // Si el servidor devolvió otro id, migrar local para evitar “fantasmas”.
                            try {
                                if (syncedEntity && syncedEntity.id && syncedEntity.id !== entityData.id) {
                                    await DB.put('inventory_items', syncedEntity, { autoBranchId: false });
                ...
                if (error && (error.status === 429 || String(error.message).includes('429'))) {
                    ...
                    scheduleSyncRetry(waitSeconds * 1000, 'rate_limit_429');

                    return;
                }

                // Si el backend indica sobrecarga temporal (ej. circuit breaker 503),
                // pausar todo el sync con jitter para evitar tormenta de requests.
                ...
                const isOverloadError =
                    ...
                    msg.includes('temporalmente no disponible') ||
                    msg.includes('circuit') ||
                    msg.includes('backoff');

                if (isOverloadError) {
                    console.warn('⚠️ Sobrecarga temporal detectada. Pausando sync para aliviar backend.');
                    this.isSyncing = false;
                    scheduleSyncRetry(15000, 'backend_overload_503');
                    return;
                }
