...
const SyncManager = {
    ...
    async init() {
        ...
        // Limpieza única de la cola pre-cargada de fábrica (solo corre una vez por dispositivo)
        const initialCleanupDone = localStorage.getItem('sync_queue_initial_cleanup_done');
        if (!initialCleanupDone) {
            try {
                await this.clearQueue();
                localStorage.setItem('sync_queue_initial_cleanup_done', '1');
                console.log('🗑️ Cola de sincronización limpiada (limpieza inicial única)');
            } catch (e) {
                console.warn('No se pudo realizar la limpieza inicial de la cola:', e);
            }
        }
        ...
        try {
            if (typeof DB !== 'undefined') {
                ...
                if (apiUrl && typeof API !== 'undefined') {
                    ...
                    // Sin token: no intentar auto-login. El usuario debe autenticarse manualmente.
    ...
    async loadQueue() {
        try {
            ...
            // Excluir items marcados como fallidos para que no se reencolen
            const queue = allQueue.filter(item => item.status !== 'failed');
            this.syncQueue = queue;
            console.log(`📋 Cola de sincronización cargada: ${queue.length} elementos (${allQueue.length - queue.length} fallidos excluidos)`);
        } catch (error) {
    ...
    async syncPending() {
        ...
        // Sin token: continuar con fallback de headers (x-username/x-branch-id). No hacer auto-login.
        if (!hasToken) {
            console.log('ℹ️ Sin token. Sincronizando con fallback de headers (x-username/x-branch-id)');
        }
        ...
        for (const item of toSync) {
            try {
                ...
                // Si tiene muchos reintentos, marcar como fallido (no eliminar para auditoría)
                if (item.retry_count >= 5) {
                    console.warn(`⚠️  Marcando como fallido después de 5 reintentos: ${item.type} - ${item.entity_id}`);
                    item.status = 'failed';
                    item.failed_at = new Date().toISOString();
                    await DB.put('sync_queue', item);
                    this.syncQueue = this.syncQueue.filter(q => q.id !== item.id);

                    // Notificar al operador con alerta persistente
                    if (typeof Utils !== 'undefined' && Utils.showNotification) {
                        Utils.showNotification(
                            `Error de sincronización: "${item.type}" no pudo enviarse al servidor. Verifica la conexión en Configuración > Sincronización.`,
                            'error'
                        );
                    }
                    // Actualizar badge de errores en topbar si existe
                    try {
                        const failedItems = await DB.query('sync_queue', 'status', 'failed') || [];
                        const badge = document.getElementById('sync-failed-badge');
                        if (badge) {
                            badge.textContent = failedItems.length;
                            badge.style.display = failedItems.length > 0 ? 'inline-flex' : 'none';
                        } else if (failedItems.length > 0) {
                            // Crear badge en el botón de sincronización del topbar
                            const syncBtn = document.getElementById('topbar-sync-now-btn');
                            if (syncBtn) {
                                let b = syncBtn.querySelector('.sync-failed-badge');
                                if (!b) {
                                    b = document.createElement('span');
                                    b.className = 'sync-failed-badge';
                                    b.id = 'sync-failed-badge';
                                    b.style.cssText = 'position:absolute;top:-4px;right:-4px;background:#ef4444;color:#fff;border-radius:50%;width:16px;height:16px;font-size:10px;display:inline-flex;align-items:center;justify-content:center;font-weight:700;';
                                    syncBtn.style.position = 'relative';
                                    syncBtn.appendChild(b);
                                }
                                b.textContent = failedItems.length;
                            }
                        }
                    } catch (e) { /* no bloquear */ }
                }
