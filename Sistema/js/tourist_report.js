...
const TouristReport = {
    ...
    async init() {
        try {
            ...
            console.error('❌ Error inicializando módulo tourist-report:', error);
            ...
            if (content) {
                content.innerHTML = `
                    <div style="padding: var(--spacing-lg); text-align: center;">
                        <h3 style="color: var(--color-danger);">Error al cargar módulo tourist-report</h3>
    ...
    async displayArrivals() {
        ...
        if (!container) {
            console.warn('⚠️ arrivals-container no encontrado, intentando crear...');
            const content = document.getElementById('module-content');
            if (content) {
                content.innerHTML = '<div id="arrivals-container"></div>';
                // Esperar un momento para que el DOM se actualice
                await Utils.delay(50);
                const newContainer = document.getElementById('arrivals-container');
                if (!newContainer) {
                    console.error('❌ No se pudo crear arrivals-container');
                    if (content) {
                        content.innerHTML = '<div style="padding: var(--spacing-lg); text-align: center; color: var(--color-danger);">Error: No se pudo inicializar el módulo de llegadas. Por favor, recarga la página.</div>';
                    }
                    return;
                }
                // Continuar con el contenedor recién creado
                return this.displayArrivals();
            } else {
    ...
    async reconcile() {
        ...
        try {
            ...
            const todaySales = sales.filter(s => {
                if (!s.created_at?.startsWith(today) || !(typeof Utils !== 'undefined' && Utils.isSaleCompleted ? Utils.isSaleCompleted(s) : (s.status === 'completada' || s.status === 'completed'))) return false;
    ...
    async getReportStats() {
        ...
        // Si no es master_admin, filtrar manualmente también - ESTRICTO
        if (!viewAllBranches && currentBranchId) {
            const normalizedBranchId = String(currentBranchId);
            reports = reports.filter(r => {
                // CRÍTICO: Excluir reportes sin branch_id para usuarios normales
                if (!r.branch_id) {
                    return false; // NO mostrar reportes sin branch_id
                }
    ...
    async reconcileForDate(date) {
        ...
        try {
            ...
            const todaySales = sales.filter(s => 
                ...
                String(s.branch_id) === String(branchId) &&
    ...
    /**
     * Elimina una llegada y su costo asociado, sincronizando con el backend
     */
    async deleteArrival(arrivalId) {
        if (!await Utils.confirm('¿Estás seguro de eliminar esta llegada? También se eliminará el costo asociado.')) {
            return;
        }
        ...
        try {
            ...
            if (isUuid && typeof API !== 'undefined' && API.baseURL && API.token && API.deleteArrival) {
                try {
                    ...
                    console.warn('⚠️ Error eliminando llegada con API (puede que no exista en servidor):', apiError.message);
                    // Continuar con eliminación local y agregar a cola de sincronización
                }
            }

            // Eliminar costo asociado si existe
            const allCosts = await DB.getAll('cost_entries') || [];
            const associatedCost = allCosts.find(c => 
                c.category === 'pago_llegadas' && 
                c.arrival_id === arrivalId
            );

            if (associatedCost) {
                // Intentar eliminar costo con API
                let costApiDeleted = false;
                if (typeof API !== 'undefined' && API.baseURL && API.token && API.deleteCost) {
                    try {
                        console.log('💰 Eliminando costo asociado con API en servidor...');
                        await API.deleteCost(associatedCost.id);
                        console.log('✅ Costo eliminado con API en servidor');
                        costApiDeleted = true;
                    } catch (costApiError) {
                        console.warn('⚠️ Error eliminando costo con API (puede que no exista en servidor):', costApiError.message);
                    }
                }
                
                // Eliminar costo local
                await DB.delete('cost_entries', associatedCost.id);
                
                // Agregar a cola de sincronización si NO se eliminó con API exitosamente
                if (typeof SyncManager !== 'undefined' && !costApiDeleted) {
                    await SyncManager.addToQueue('cost_entry', associatedCost.id, 'delete');
                    console.log('📤 Costo agregado a cola de sincronización para eliminar en servidor');
                }
            }

            // Eliminar la llegada de la base de datos local
            await DB.delete('agency_arrivals', arrivalId);

            // Agregar llegada a cola de sincronización si NO se eliminó con API exitosamente
            if (typeof SyncManager !== 'undefined' && !apiDeleted) {
                await SyncManager.addToQueue('agency_arrival', arrivalId, 'delete');
