...
Object.assign(POS, {
    ...
    async calculatePayments() {
        try {
            ...
            const _fbUsd = parseFloat(localStorage.getItem('last_exchange_rate_usd') || localStorage.getItem('daily_exchange_rate') || 20);
            const _fbCad = parseFloat(localStorage.getItem('last_exchange_rate_cad') || 15);
            let exchangeRate = _fbUsd;
            let exchangeRateCadValue = _fbCad;
            try {
                if (typeof ExchangeRates !== 'undefined' && ExchangeRates.getExchangeRate) {
                    ...
                    exchangeRate = parseFloat(exchangeRates?.usd || _fbUsd) || _fbUsd;
                    exchangeRateCadValue = parseFloat(exchangeRates?.cad || _fbCad) || _fbCad;
                    ...
                    exchangeRate = parseFloat(rate?.usd_to_mxn || rate?.usd || _fbUsd) || _fbUsd;
                    exchangeRateCadValue = parseFloat(rate?.cad_to_mxn || rate?.cad || _fbCad) || _fbCad;
                }
            } catch (e) {
                // Usar fallback basado en último tipo real conocido
            }
    ...
    async completeSale() {
        ...
        // Verificar que ningún ítem del carrito esté en reparación o apartado por otra venta
        for (const cartItem of this.cart) {
            const currentItem = await DB.get('inventory_items', cartItem.id);
            if (currentItem && currentItem.status === 'reparacion') {
                Utils.showNotification(
                    `"${currentItem.name || cartItem.name}" está en reparación y no puede venderse hasta que se entregue`,
                    'error'
                );
                return;
            }
            if (currentItem && currentItem.status === 'apartada') {
                Utils.showNotification(
                    `"${currentItem.name || cartItem.name}" está apartado por otra venta activa y no puede venderse`,
                    'error'
                );
                return;
            }
        ...
        const today = Utils.formatDate(new Date(), 'YYYY-MM-DD');
        const _saleFbUsd = parseFloat(localStorage.getItem('last_exchange_rate_usd') || localStorage.getItem('daily_exchange_rate') || 20);
        let exchangeRate = _saleFbUsd;
        try {
            if (typeof ExchangeRates !== 'undefined' && ExchangeRates.getExchangeRate) {
                const exchangeRates = await ExchangeRates.getExchangeRate(today);
                exchangeRate = parseFloat(exchangeRates?.usd || _saleFbUsd) || _saleFbUsd;
            } else if (typeof API !== 'undefined' && API.baseURL && API.token && API.getExchangeRateByDate) {
                const r = await API.getExchangeRateByDate(today);
                exchangeRate = parseFloat(r?.usd_to_mxn || r?.usd || _saleFbUsd) || _saleFbUsd;
            }
        } catch (e) {
            // Usar fallback basado en último tipo real conocido
        }
