...
const Dashboard = {
    ...
    async loadDashboard(showAllBranches = false) {
        try {
            ...
            // SIEMPRE intentar cargar desde API si está disponible (tiempo real)
            if (typeof API !== 'undefined' && API.baseURL && API.token && API.getDashboardMetrics) {
                try {
                    console.log('📊 Cargando métricas del dashboard desde API (tiempo real)...');
                    // Para vista consolidada (master_admin sin filtro), enviar null explícitamente
                    ...
                    // Guardar en IndexedDB como caché (solo para offline)
                    for (const sale of allSales) {
                        try {
                        await DB.put('sales', sale);
                        } catch (e) {
                            console.warn('Error guardando venta en caché:', e);
                        }
                    }
                    for (const arrival of todayArrivals) {
                        ...
                        await DB.put('agency_arrivals', arrival);
                            ...
                            console.warn('Error guardando llegada en caché:', e);
                        }
                    }
                    
                    console.log(`✅ Métricas cargadas desde API: ${allSales.length} ventas, ${todayPassengers} pasajeros`);
                } catch (apiError) {
                    console.warn('⚠️ Error cargando métricas desde API, usando modo local:', apiError);
                    ...
                    todayArrivals = arrivals.filter(a => {
                        // CRÍTICO: Excluir arrivals sin branch_id cuando se filtra por sucursal específica
                        if (!viewAllBranches && filterBranchId) {
                            if (!a.branch_id) {
                                return false; // NO mostrar arrivals sin branch_id
                            }
                            return String(a.branch_id) === String(filterBranchId);
                        }
                        // Para master_admin sin filtro específico, incluir todos (incluidos sin branch_id)
                        return (a.passengers > 0 && a.units > 0);
                    });
    ...
    async calculateBranchBreakdown(branches, todayStr, today) {
        ...
        for (const branch of branches.filter(b => b.active)) {
            ...
            const todayBranchSales = branchSales.filter(s =>
                s.created_at && s.created_at.startsWith(todayStr) && (typeof Utils !== 'undefined' && Utils.isSaleCompleted ? Utils.isSaleCompleted(s) : (s.status === 'completada' || s.status === 'completed'))
            );
    ...
    calculateBranchBreakdownFromSales(branches, completedSales, todayStr, today, todayArrivals = []) {
        ...
        for (const branch of (branches || []).filter(b => b && b.active)) {
            const todayBranchSales = completedSales.filter(s =>
                s.branch_id && String(s.branch_id) === String(branch.id) && s.created_at && s.created_at.startsWith(todayStr)
            );
            ...
            breakdown.push({
                branch,
                todaySales: todayBranchSales.reduce((sum, s) => sum + (s.total || 0), 0),
                todayTickets: todayBranchSales.length,
                todayPassengers: todayBranchPassengers,
                monthSales: monthBranchSales.reduce((sum, s) => sum + (s.total || 0), 0),
                monthTickets: monthBranchSales.length
            });
        }
        return breakdown;
    ...
    async renderDashboard(data) {
        ...
        container.innerHTML = `
            ${viewAllInfo}
            <!-- Filtro de Sucursal (solo para master_admin) -->
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: var(--spacing-lg); margin-bottom: var(--spacing-lg);">
                <div class="module" style="padding: var(--spacing-md); background: var(--color-bg-card); border-radius: var(--radius-md); border: 1px solid var(--color-border-light);">
                    <h3 style="margin-bottom: var(--spacing-md); font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">
                        <i class="fas fa-truck"></i> Estadísticas de Proveedores
                    </h3>
                    <div style="background: var(--color-bg-secondary); padding: var(--spacing-md); border-radius: var(--radius-md); margin-top: var(--spacing-sm);">
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--spacing-md);">
                            <div style="text-align: center;">
                                <div style="font-size: 24px; font-weight: 700; color: var(--color-primary); margin-bottom: 4px;">
                                    ${data.suppliersStats.totalSuppliers || 0}
                                </div>
                                <div style="font-size: 11px; color: var(--color-text-secondary);">Proveedores Activos</div>
                            </div>
                            <div style="text-align: center;">
                                <div style="font-size: 24px; font-weight: 700; color: var(--color-accent); margin-bottom: 4px;">
                                    ${data.suppliersStats.recentSuppliers || 0}
                                </div>
                                <div style="font-size: 11px; color: var(--color-text-secondary);">Nuevos (30 días)</div>
                            </div>
                            <div style="text-align: center;">
                                <div style="font-size: 24px; font-weight: 700; color: var(--color-success); margin-bottom: 4px;">
                                    ${data.suppliersStats.totalPurchases || 0}
                                </div>
                                <div style="font-size: 11px; color: var(--color-text-secondary);">Compras (30 días)</div>
                            </div>
                            <div style="text-align: center;">
                                <div style="font-size: 20px; font-weight: 700; color: var(--color-primary); margin-bottom: 4px;">
                                    ${Utils.formatCurrency(data.suppliersStats.totalPurchaseAmount || 0)}
                                </div>
                                <div style="font-size: 11px; color: var(--color-text-secondary);">Total Compras</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="module" style="padding: var(--spacing-md); background: var(--color-bg-card); border-radius: var(--radius-md); border: 1px solid var(--color-border-light);">
                    <h3 style="margin-bottom: var(--spacing-md); font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">
                        <i class="fas fa-trophy"></i> Top 5 Proveedores (30 días)
                    </h3>
                    <div style="background: var(--color-bg-secondary); padding: var(--spacing-md); border-radius: var(--radius-md); margin-top: var(--spacing-sm);">
                                    ...
                                    return `
                                        <div>
                                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                                                <div>
                                                    <span style="font-weight: 600; color: var(--color-primary); font-size: 10px;">#${idx + 1}</span>
                                                    <span style="margin-left: var(--spacing-xs); font-weight: 600; font-size: 11px;">${supplier.code || ''} ${supplier.code && supplier.name ? '- ' : ''}${supplier.name || 'Sin nombre'}</span>
                                                </div>
                                                <span style="font-size: 12px; font-weight: 600;">${Utils.formatCurrency(supplier.total_amount || 0)}</span>
                                            </div>
                                            <div style="width: 100%; height: 18px; background: var(--color-border-light); border-radius: var(--radius-full); overflow: hidden; position: relative;">
                                                <div style="width: ${width}%; height: 100%; background: linear-gradient(90deg, #f59e0b 0%, #ef4444 100%); transition: width 0.5s;"></div>
                                                <div style="position: absolute; right: var(--spacing-xs); top: 50%; transform: translateY(-50%); font-size: 9px; color: var(--color-text-secondary);">
                                                    ${supplier.purchase_count || 0} compras
                                                </div>
                                            </div>
                                        </div>
                                    `;
                        ...
                        ` : '<p style="text-align: center; color: var(--color-text-secondary); padding: var(--spacing-md); font-size: 11px;">No hay compras registradas</p>'}
                    </div>
                </div>
            </div>
            ` : ''}
