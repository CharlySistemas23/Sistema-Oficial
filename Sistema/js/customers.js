...
const Customers = {
    ...
    async loadCustomers() {
        try {
            ...
            const fromIdb = await DB.getAll('customers', null, null, { filterByBranch: false }) || [];
            ...
            if (filterBranchId) {
                ...
                cachedFiltered = fromIdb.filter(c => c.branch_id && norm(c.branch_id) === cb);
            }
            const hasCache = cachedFiltered.length > 0;
            const hasApi = typeof API !== 'undefined' && API.baseURL && API.token && API.getCustomers;

            let customers = [];
            let sales = [];

            if (hasCache) {
                customers = cachedFiltered;
                console.log(`⚡ [Cache-First] ${customers.length} clientes desde IndexedDB`);
                if (hasApi) {
                    Promise.resolve().then(async () => {
                        try {
                            const fresh = await API.getCustomers(filterBranchId ? { branch_id: filterBranchId } : {});
                            for (const c of fresh) {
                                try {
                                    await DB.put('customers', { ...c, server_id: c.id, sync_status: 'synced' }, { autoBranchId: false });
                                } catch (e) {}
                            }
                            const beforeIds = new Set(customers.map(c => c.id));
                            const afterIds = new Set(fresh.map(c => c.id));
                            if (beforeIds.size !== afterIds.size || [...beforeIds].some(id => !afterIds.has(id))) {
                                await this.loadCustomers();
                            }
                        } catch (e) { console.warn('Sync customers background:', e); }
                    }).catch(e => console.warn('Sync customers background:', e));
                }
            } else if (hasApi) {
                const content = document.getElementById('customers-content') || document.getElementById('module-content');
                if (content) content.innerHTML = '<div style="padding: var(--spacing-lg); text-align: center;"><i class="fas fa-spinner fa-spin"></i> Cargando clientes...</div>';
                try {
            ...
            const hasApiSales = typeof API !== 'undefined' && API.baseURL && API.token && API.getSales;
            if (hasApiSales) {
                if (sales.length > 0) {
                    Promise.resolve().then(async () => {
                        try {
    ...
    async confirmBulkDelete() {
        for (const id of this.selectedCustomers) {
            ...
            if (typeof API !== 'undefined' && API.baseURL && API.token && API.deleteCustomer) {
                ...
                // Encolar para sincronización cuando vuelva la conexión
                const customer = await DB.get('customers', id);
                if (customer) {
                    try {
                        await DB.put('sync_deleted_items', {
                            id,
                            entity_type: 'customer',
                            metadata: { id, name: customer.name, branch_id: customer.branch_id, deleted_at: new Date().toISOString() },
                            deleted_at: new Date().toISOString()
                        });
                        await SyncManager.addToQueue('customer', id, 'delete');
                    } catch (syncError) {
                        console.error('Error guardando metadata para sincronización:', syncError);
                    }
                }
