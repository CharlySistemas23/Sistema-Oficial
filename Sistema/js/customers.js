        ...
        try {
            ...
            if (typeof API !== 'undefined' && API.baseURL && API.token && API.getCustomers) {
                try {
                    ...
                    customers = await API.getCustomers(viewAllBranches ? '' : (currentBranchId ? `?branch_id=${currentBranchId}` : ''));
                    ...
                    console.log(`✅ ${customers.length} clientes cargados desde API`);
                } catch (apiError) {
            ...
            // Filtrado adicional por sucursal si no es admin
            if (!viewAllBranches && currentBranchId) {
                const normalizedBranchId = String(currentBranchId);
                customers = customers.filter(c => {
                    if (!c.branch_id) return true; // Incluir clientes sin sucursal
            ...
            if (typeof API !== 'undefined' && API.baseURL && API.token && API.getSales) {
                try {
                    sales = await API.getSales({ branch_id: viewAllBranches ? null : currentBranchId });
                } catch (apiError) {
