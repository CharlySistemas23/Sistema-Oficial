        ...
        try {
            ...
            if (typeof API !== 'undefined' && API.baseURL && API.token && API.getCustomers) {
                try {
                    ...
                    const queryParams = filterBranchId ? `?branch_id=${filterBranchId}` : '';
                    customers = await API.getCustomers(queryParams);
                    
                    // CRÍTICO: Aplicar filtro estricto DESPUÉS de recibir de API
                    // Esto asegura que clientes sin branch_id se excluyan cuando se filtra por sucursal específica
                    if (filterBranchId) {
                    ...
                    console.log(`✅ ${customers.length} clientes cargados desde API (después de filtro estricto)`);
                } catch (apiError) {
                    console.warn('Error cargando clientes desde API, usando modo local:', apiError);
                    // Modo offline: filtrar por sucursal
            ...
            if (!customersList) {
                console.warn('⚠️ customers-list no encontrado, reconfigurando UI...');
                this.setupEventListeners();
    ...
    async exportCustomers() {
        ...
        try {
            ...
            const exportData = filteredCustomers.map(c => {
                const customerSales = filteredSales.filter(s => s.customer_id === c.id && s.status === 'completada');
                const totalSpent = customerSales.reduce((sum, s) => sum + (s.total || 0), 0);
