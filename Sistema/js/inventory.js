...
const Inventory = {
        ...
        try {
            ...
            if (!useFastPath && typeof API !== 'undefined' && API.baseURL && API.token && API.getInventoryItems) {
                try {
                    ...
                    // Limpiar claves con valor undefined
                        ...
                        try {
                            ...
                            if (fromIdb.length > 0) {
                                ...
                                if (allItemsRaw.length > 0) {
                                    ...
                                    this._lastLoadEmptyFromServer = false; // mostrando caché, no estado vacío del servidor
                                }
                            }
                        } catch (e) {
                    ...
                    if (apiReturnedZero && allItemsRaw.length === 0) {
                        this._lastLoadEmptyFromServer = true;
                    } else if (!apiReturnedZero) {
                        this._lastLoadEmptyFromServer = false;
                    }
                } catch (apiError) {
            ...
            // Barra de paginación (solo en modo online paginado)
            if (paginatedResponse) {
                this.renderPaginationControls(this.totalItems, this.currentPage, this.totalPages);
            } else {
                // Quitar barra si estaba de antes
    ...
    async loadSupplierDropdown(selectedSupplierId = null) {
        ...
        try {
            // Cargar proveedores desde IndexedDB o API
            let suppliers = [];
            if (typeof Suppliers !== 'undefined' && Suppliers.loadSuppliers) {
                // Si el módulo Suppliers está disponible, usar su función
                ...
                // Fallback: cargar desde IndexedDB directamente
                suppliers = await DB.getAll('suppliers') || [];
            }
