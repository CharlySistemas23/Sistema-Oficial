...
const Inventory = {
    ...
    async loadInventory() {
        ...
        try {
            ...
            // 1. Si hay un filtro específico en el dropdown, usarlo
            // 2. Si es master_admin y no hay filtro, mostrar todos (null)
            // 3. Si NO es master_admin, usar currentBranchId
            ...
            if (!viewAllBranches && filterBranchId) {
                ...
                // Master admin sin filtro = mostrar todos (incluyendo items sin branch_id)
                console.log(`📍 Sin filtro de sucursal: ${items.length} items (master_admin - todas las sucursales)`);
            } else {
                // Sin sucursal asignada = mostrar 0 items
                items = [];
