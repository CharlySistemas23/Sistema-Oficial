...
const SyncUI = {
    ...
    async updateServerStatus() {
        ...
        try {
            ...
            if (isConnected) {
                ...
                let reason = 'Modo offline: Funcionando localmente';
                if (!hasToken) {
                    reason = 'Modo offline: Sin autenticación. El sistema funciona localmente.';
                } else if (!hasSocket) {
                    reason = 'Reconectando con el servidor...';
                }
    ...
    async updateSyncStatus() {
        ...
        try {
            ...
            // Verificar conexión: URL configurada Y token disponible (en API o localStorage)
            const hasToken = (typeof API !== 'undefined' && API.token) || localStorage.getItem('api_token');
            ...
            if (!isConnected) {
                ...
                if (detailDiv) {
                    detailDiv.textContent = 'Servidor no configurado';
                }
                statusDiv.innerHTML = `
                    <div style="text-align: center;">
