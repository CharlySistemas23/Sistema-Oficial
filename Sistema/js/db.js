...
const DB = {
    db: null,
    dbName: 'opal_pos_db',
    version: 14, // v14: índice updated_at en inventory_items para sync incremental
    ...
    createStores(db, event = null) {
        ...
        if (!db.objectStoreNames.contains('inventory_items')) {
            ...
            // v14: agregar índice updated_at si aún no existe (upgrade en caliente)
            const transaction = event.target.transaction;
            if (transaction) {
                const invStore = transaction.objectStore('inventory_items');
                if (!invStore.indexNames.contains('updated_at')) {
                    invStore.createIndex('updated_at', 'updated_at', { unique: false });
                }
            }
        }
        ...
        // Temp Quick Captures (Capturas Rápidas Temporales del Día)
        if (!db.objectStoreNames.contains('temp_quick_captures')) {
            const tempCaptureStore = db.createObjectStore('temp_quick_captures', { keyPath: 'id' });
            tempCaptureStore.createIndex('date', 'date', { unique: false });
            tempCaptureStore.createIndex('branch_id', 'branch_id', { unique: false });
            tempCaptureStore.createIndex('seller_id', 'seller_id', { unique: false });
            tempCaptureStore.createIndex('guide_id', 'guide_id', { unique: false });
            tempCaptureStore.createIndex('agency_id', 'agency_id', { unique: false });
            tempCaptureStore.createIndex('created_at', 'created_at', { unique: false });
        }
        ...
        // Historical Reports (Reportes históricos agregados)
        if (!db.objectStoreNames.contains('historical_reports')) {
            const histStore = db.createObjectStore('historical_reports', { keyPath: 'id' });
            histStore.createIndex('period_type', 'period_type', { unique: false });
            histStore.createIndex('date_from', 'date_from', { unique: false });
        ...
        // Supplier Payments (Pagos de Proveedores)
        if (!db.objectStoreNames.contains('supplier_payments')) {
            const paymentStore = db.createObjectStore('supplier_payments', { keyPath: 'id' });
            paymentStore.createIndex('supplier_id', 'supplier_id', { unique: false });
            paymentStore.createIndex('branch_id', 'branch_id', { unique: false });
            paymentStore.createIndex('status', 'status', { unique: false });
            paymentStore.createIndex('reference_number', 'reference_number', { unique: false });
            paymentStore.createIndex('due_date', 'due_date', { unique: false });
        }

        // Payment Invoices (Recibos de Pago - Historial de Pagos)
        if (!db.objectStoreNames.contains('payment_invoices')) {
            const receiptStore = db.createObjectStore('payment_invoices', { keyPath: 'id' });
            receiptStore.createIndex('supplier_payment_id', 'supplier_payment_id', { unique: false });
            receiptStore.createIndex('branch_id', 'branch_id', { unique: false });
            receiptStore.createIndex('receipt_number', 'receipt_number', { unique: true });
            receiptStore.createIndex('payment_date', 'payment_date', { unique: false });
                        ...
                        if (branchId && options.branchIdField) {
                            ...
                            if (shouldFilter) {
                                ...
                                if (filtered.length === 0) {
                                    // CRÍTICO: Si hay un filtro específico de sucursal (filterByBranch === true o branchId está definido),
                                    // NO hacer fallback a "todos los items". El filtrado estricto debe respetarse.
                                    // Solo hacer fallback si NO hay un filtro específico (cuando se quiere ver todas las sucursales)
                                    const hasSpecificBranchFilter = options.filterByBranch === true;
                                    
                                    // SOLO master_admin puede ver items sin branch_id como fallback, PERO solo si NO hay filtro específico
                                    if (isAdmin && options.includeNull !== false && !hasSpecificBranchFilter) {
                                    ...
                                    if (itemsWithoutBranch.length > 0) {
                                        ...
                                        filtered = itemsWithoutBranch;
                                            ...
                                            // Si tampoco hay items sin branch_id, master_admin puede ver todos (solo sin filtro específico)
                                            console.log(`[DB] No hay items para sucursal ${branchId} ni sin sucursal, master_admin viendo todos (${results.length} items) - SIN filtro específico`);
                                            filtered = results;
                                        }
                                    } else {
                                        // Con filtro específico o usuario normal: NO mostrar items sin branch_id ni todos (aislamiento estricto)
                                        console.log(`[DB] No hay items para sucursal ${branchId}. Mostrando 0 (filtrado estricto)`);
                                        filtered = [];
                                    }
                                }
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (indexName && !store.indexNames.contains(indexName)) {
                    // Usar console.debug en lugar de console.warn para reducir ruido en consola
                    // El fallback funciona correctamente, solo es informativo
                    // console.debug(`Índice ${indexName} no existe en el store ${storeName}, usando getAll y filtrando manualmente`);
