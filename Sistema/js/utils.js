...
const Utils = {
    ...
    /**
     * Indica si una venta se considera completada (para totales, historial, dashboard).
     * Acepta tanto 'completed' (backend/API) como 'completada'/'completado' (legado).
     * No elimina ni modifica datos; solo criterio de lectura unificado.
     */
    isSaleCompleted(sale) {
        if (!sale || sale.status == null) return false;
        const s = String(sale.status).toLowerCase();
        return s === 'completed' || s === 'completada' || s === 'completado';
    },

    /**
     * Devuelve el total de una venta como número (para sumas y totales).
     * Siempre usa parseFloat para evitar concatenación si total viene como string de API/IDB.
     * Usar en totales del día, del mes, por sucursal, etc., para que siempre se sumen todas las ventas.
     */
    getSaleTotal(sale) {
        if (sale == null) return 0;
        const t = sale.total ?? sale.total_amount;
    ...
    normalizeDateKey(val) {
        ...
        if (typeof val === 'string') {
            ...
            if (!Number.isNaN(d.getTime())) return this.formatDate(d, 'YYYY-MM-DD');
            return '';
        }
    ...
    normalizeBranchId(val) {
        if (val == null) return '';
        return String(val).trim().toLowerCase();
    },

    normalizeCategoryKey(val) {
        if (!val) return '';
        return String(val).trim().toLowerCase().replace(/\s+/g, '_');
    },

    normalizeCostType(val) {
        if (!val) return '';
        return String(val).trim().toLowerCase();
    },

    isArrivalCostCategory(category) {
        return this.normalizeCategoryKey(category) === 'pago_llegadas';
    },

    isSalesCostCategory(category) {
        const key = this.normalizeCategoryKey(category);
        return key === 'costo_ventas' || key === 'comisiones' || key === 'comisiones_bancarias';
    },

    isOperationalCostCategory(category) {
        return !this.isSalesCostCategory(category) && !this.isArrivalCostCategory(category);
    },

    getSaleItemCommission(item) {
        if (!item) return 0;
        if (item.commission_amount != null && item.commission_amount !== '') {
            return this.parseAmount(item.commission_amount);
        }
        return this.parseAmount(item.seller_commission) + this.parseAmount(item.guide_commission);
    },
