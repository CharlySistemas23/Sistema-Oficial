...
const App = {
                    ...
                    if (typeof Customers !== 'undefined') {
                        if (!Customers.initialized) {
                            ...
                            const needsReconfig = !content || 
                                ...
                                content.innerHTML.includes('Cargando módulo') || 
                                !customersList ||
                                (customersList && customersList.children.length === 0 && !content.querySelector('.customers-module'));
    ...
    async loadArrivalRateRules() {
        try {
            ...
            // Hay aproximadamente 23 reglas en total (3 TANITOURS + 1 TRAVELEX + 6 DISCOVERY + 1 VERANOS + 5 TB + 5 TTF + 2 TROPICAL)
            ...
            if (existingRules.length > 0 && existingRules.length < 20) {
                console.log(`Reglas incompletas detectadas (${existingRules.length} < 20), recargando todas las reglas...`);
