...
router.post('/agencies', requireMasterAdmin, [
  ...
  try {
    ...
    const { code, name, barcode, active = true } = req.body;
...
router.get('/guides', async (req, res) => {
  try {
    const { search, agency_id, active } = req.query;
...
router.post('/guides', requireMasterAdmin, [
  ...
  try {
    ...
    const { code, name, barcode, agency_id, active = true } = req.body;

    const result = await query(
      `INSERT INTO catalog_guides (code, name, barcode, agency_id, active)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [code, name, barcode, agency_id, active]
    );
    ...
    await query(
      ...
      [req.user.id, result.rows[0].id, JSON.stringify({ code, name, agency_id })]
    );
...
router.put('/guides/:id', requireMasterAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { code, name, barcode, agency_id, active } = req.body;
    ...
    const result = await query(
      `UPDATE catalog_guides
       ...
       WHERE id = $6
       RETURNING *`,
      [code, name, barcode, agency_id, active, id]
    );
