...
router.post('/:id/pay', requireBranchAccess, async (req, res) => {
  try {
    const { id } = req.params;
    const {
      payment_amount, payment_date, payment_method, payment_reference, notes
    } = req.body;
    ...
    await query(
      `INSERT INTO payment_invoices (
        supplier_payment_id, payment_date, payment_amount, payment_method, payment_reference, notes, created_by
      )
      VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [id, payment_date, paymentAmount, payment_method, payment_reference || null, notes || null, req.user.id || null]
    );
    ...
    const result = await query(
      ...
      WHERE id = $4
      RETURNING *`,
      [newPaidAmount, newStatus, payment_date, id]
    );
