...
async function verifyAndCreateAdmin() {
  ...
  try {
    ...
    const adminCheck = await pool.query(`SELECT id, username, role, active FROM users WHERE username = 'admin' LIMIT 1`);
    ...
    // Verificar también usuario master_admin (del schema inicial)
    const masterAdminCheck = await pool.query(`SELECT id, username, role FROM users WHERE username = 'master_admin' LIMIT 1`);
    if (masterAdminCheck.rows.length > 0) {
      console.log('⚠️  También existe usuario "master_admin" (del schema inicial)');
      console.log(`   - ID: ${masterAdminCheck.rows[0].id}`);
      console.log(`   - Rol: ${masterAdminCheck.rows[0].role}`);
      console.log('   💡 Puedes usar cualquiera de los dos usuarios\n');
    }
    ...
    const finalCheck = await pool.query(`
      ...
      FROM users u
      LEFT JOIN employees e ON u.employee_id = e.id
      WHERE u.username = 'admin' AND u.active = true
    `);
