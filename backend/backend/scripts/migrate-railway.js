...
// Ejecutar con: railway ssh --service Sistema-Oficial "cd /app && node scripts/migrate-railway.js"
...
-- Clientes (DEBE CREARSE ANTES DE repairs y sales)
...
-- Repairs (AHORA customers YA EXISTE)
...
-- Ventas (AHORA customers YA EXISTE)
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      if (statement.trim().length > 0) {
        try {
          ...
          if (i % 10 === 0) {
            console.log(`  Procesando statement ${i + 1}/${statements.length}...`);
          }
        } catch (error) {
